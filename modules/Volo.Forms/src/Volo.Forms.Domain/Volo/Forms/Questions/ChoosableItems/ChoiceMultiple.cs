...
public class ChoiceMultiple : QuestionBase, IChoosable, IRequired, IHasOtherOption
{
    ...
    public ChoiceMultiple(Guid id, Guid? tenantId = null) : base(id, tenantId)
    {
        Choices = new Collection<Choice>();
    }
    ...
    {
        return QuestionTypes.ChoiceMultiple;
    }
    ...
    public void AddChoiceOther(Guid id, int index = 0, Guid? tenantId = null)
    {
        SetOtherOption(true);
        var newIndex = index == 0 ? Choices.Count + 1 : index;
        AddChoice(id: id, index: newIndex, ChoiceConsts.OtherChoice, tenantId: tenantId);
    }
