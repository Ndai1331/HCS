...
public partial class ProjectTaskAssignmentsAppService : ProjectTaskAssignmentsAppServiceBase, IProjectTaskAssignmentsAppService
{
    ...
    // Restrict the user picker to the current user's organization-unit scope (self + parents + children,
    // across every OU the user belongs to). Admins can pick any user. Search matches (surname + " " + name).
    public override async Task<PagedResultDto<LookupDto<Guid>>> GetIdentityUserLookupAsync(LookupRequestDto input)
    {
        ...
        if (!CurrentUser.IsAdminRole())
        {
            ...
            var scopeOuIds = await _workflowAssigneeResolver.GetOrganizationUnitScopeWithDescendantsForUserAsync(CurrentUser.Id.Value);
