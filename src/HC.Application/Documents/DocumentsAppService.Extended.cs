...
public class DocumentsAppService : DocumentsAppServiceBase, IDocumentsAppService
{
    ...
    public DocumentsAppService(IDocumentRepository documentRepository, DocumentManager documentManager, IDistributedCache<DocumentDownloadTokenCacheItem, string> downloadTokenCache, IRepository<HC.MasterDatas.MasterData, Guid> masterDataRepository, IRepository<HC.Units.Unit, Guid> unitRepository, IRepository<HC.Workflows.Workflow, Guid> workflowRepository, ILogger<DocumentsAppServiceBase> logger, IDocumentHistoryRepository documentHistoryRepository, DocumentHistoryManager documentHistoryManager, INotificationRepository notificationRepository, INotificationReceiverRepository notificationReceiverRepository, IDistributedEventBus distributedEventBus, IRepository<HC.Documents.Document, Guid> documentRepository2, IRepository<HC.UserDepartments.UserDepartment, Guid> userDepartmentRepository, IDocumentAssignmentRepository documentAssignmentRepository, DocumentAssignmentManager documentAssignmentManager, IRepository<IdentityUser, Guid> identityUserRepository, IRepository<Department, Guid> departmentRepository, IDocumentWorkflowInstanceRepository documentWorkflowInstanceRepository, IDocumentFileRepository documentFileRepository, IBlobContainer blobContainer, IPdfStampingService pdfStampingService, IUserSignatureRepository userSignatureRepository, IWorkflowSigningExecutionService workflowSigningExecutionService, IDistributedCache<MasterDataCodeCacheItem, string> masterDataCodeCache) : base(documentRepository, documentManager, downloadTokenCache, masterDataRepository, unitRepository, workflowRepository, logger)
    {
        ...
        _masterDataCodeCache = masterDataCodeCache;
    }
    //</suite-custom-code-autogenerated>
    ...
    private async Task PopulateSentToMeDisplayNamesAsync(List<DocumentWithNavigationPropertiesDto> dtos)
    {
        ...
        // Project only the columns needed for the display name; IdentityUser is wide
        // (claims, lockout, security stamp, ...) so loading the full entity is wasteful.
        ...
        if (userIds.Count > 0)
        {
            ...
            var userRows = await AsyncExecuter.ToListAsync(userQ);
            userDict = userRows.ToDictionary(
                u => u.Id,
                u =>
                {
    ...
    private async Task<(byte[]? SignatureImageBytes, string? SignerFullName)> TryGetApprovalSignatureStampAsync(Guid currentUserId)
    {
        var user = await _identityUserRepository.GetAsync(currentUserId);
        var signerFullName = GetIdentityUserFullDisplayName(user);

        try
        {
            var now = Clock.Now;
            var signatureQuery = (await _userSignatureRepository.GetQueryableAsync())
                .Where(x => x.IdentityUserId == currentUserId
                            && x.SignType == nameof(SignType.ELECTRONIC)
                            && x.IsActive)
    ...
    /// Looks up MasterData by Code and Type = "TRANG_THAI_VB" (cached in IDistributedCache).
    /// </summary>
    private Task UpdateDocumentStatusAsync(Guid documentId, DocumentStatusCode statusCode)
    {
        return UpdateDocumentStatusAsync(documentId, statusCode, document: null);
    }

    /// <summary>
    /// Update document status when the caller already loaded the Document entity (avoids one extra GetAsync round-trip).
    /// </summary>
    private async Task UpdateDocumentStatusAsync(Guid documentId, DocumentStatusCode statusCode, Document? document)
    {
        try
        {
            var code = statusCode.GetCode();
            var statusType = MasterDataType.Status.GetTypeValue();

            var statusId = await ResolveMasterDataIdByCodeAsync(statusType, code);
            if (statusId == null)
            {
                _logger.LogWarning("MasterData with Code='{Code}' and Type='{Type}' not found. Document status will not be updated.", code, statusType);
                return;
            }

            document ??= await _documentRepository.GetAsync(documentId);
            if (document.StatusId == statusId.Value)
            {
                return; // No-op when already in the target status.
            }

            document.StatusId = statusId.Value;
            await _documentRepository.UpdateAsync(document);

            _logger.LogInformation("Document status updated to {Code}: DocumentId={DocumentId}, StatusId={StatusId}",
                code, documentId, statusId.Value);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error updating document status to {Code}: DocumentId={DocumentId}",
                statusCode.GetCode(), documentId);
            // Don't throw - parent flow (sign / approve / send) must not fail because of a status display update.
        }
    }

    /// <summary>
    /// Resolves a MasterData Id by (Type, Code) using a distributed cache; falls back to a single DB query on miss.
    /// </summary>
    private async Task<Guid?> ResolveMasterDataIdByCodeAsync(string type, string code)
    {
        ...
        var cacheKey = BuildMasterDataCodeCacheKey(type, code);
        var cached = await _masterDataCodeCache.GetAsync(cacheKey);
        if (cached != null && cached.Id != Guid.Empty)
        {
            return cached.Id;
        }

        var query = (await _masterDataRepository.GetQueryableAsync())
            .Where(x => x.Code == code && x.Type == type)
            .Select(x => new { x.Id });
        var match = await AsyncExecuter.FirstOrDefaultAsync(query);
        if (match == null)
        {
            return null;
        }

        await _masterDataCodeCache.SetAsync(cacheKey,
            new MasterDataCodeCacheItem { Id = match.Id, Code = code, Type = type },
            new DistributedCacheEntryOptions { AbsoluteExpirationRelativeToNow = TimeSpan.FromHours(1) });
        return match.Id;
    }

    private string BuildMasterDataCodeCacheKey(string type, string code)
    {
        var tenant = CurrentTenant.Id?.ToString("N") ?? "host";
        return $"{tenant}:{type}:{code}";
    }
}
