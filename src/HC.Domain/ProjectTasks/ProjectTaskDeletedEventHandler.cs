...
public class ProjectTaskDeletedEventHandler : ILocalEventHandler<EntityDeletedEventData<ProjectTask>>, ITransientDependency
{
    ...
    public async Task HandleEventAsync(EntityDeletedEventData<ProjectTask> eventData)
    {
        ...
        try
        {
            ...
            var calendarEvents = await _calendarEventRepository.GetListAsync(
                x => x.RelatedType == RelatedType.TASK.ToString() && x.RelatedId == relatedId);
