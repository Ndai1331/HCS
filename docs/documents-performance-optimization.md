...
## 4. Đợt 3 — Long-term (kiến trúc) — đang triển khai / mở rộng

Chi tiết kỹ thuật: `[docs/documents-performance-phase3.md](documents-performance-phase3.md)`.


| Hạng mục                                                                                                                       | Trạng thái                                                                     |
| ------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------ |
| **Approve-with-note** → `IBackgroundJobManager` + `DocumentBackgroundOperation` + ETO RabbitMQ + SignalR + toast góc dưới phải | ✅ Done                                                                         |
| **Ký số / LibreOffice** → job tương tự                                                                                         | ⏳ Kế hoạch (cùng pattern)                                                      |
| **Worker process** tách khỏi API web                                                                                           | 📄 Hướng dẫn trong `documents-performance-phase3.md` (chưa có project riêng)   |
| **Materialized view** SentToMe                                                                                                 | 📄 SQL mẫu `docs/sql/phase3_sent_to_me_materialized_view.example.sql`          |
| **Outbox** `AppNotificationOutboxes` + worker cảnh báo khi có bản ghi pending                                                  | ✅ Bảng + worker stub (chưa ghi outbox từ nghiệp vụ — tránh đổi hành vi notify) |


Migration EF: `Phase3_DocumentBackgroundOperation_And_Outbox` (bảng `AppDocumentBackgroundOperations`, `AppNotificationOutboxes`).
...
- 2026-04-21: Đợt 3 (phần 1) — `queue-approve-with-note` + job `ApproveWithNoteBackgroundJob`, tiến trình qua `DocumentBackgroundOperationProgressEto` → SignalR → toast góc dưới phải; bảng outbox + worker stub; tài liệu & SQL mẫu MV SentToMe. Chạy `dotnet ef database update` (migration `Phase3_DocumentBackgroundOperation_And_Outbox`).